"""Compara a LEITURA DE COMPROVANTE bancario (Pix/TED/DOC/boleto): Sonnet vs Haiku.

Comprovante NAO e' cupom fiscal: nao tem itens, nao tem chave/QR, nao alimenta o
banco de precos. So' precisamos de 4 campos: tipo (receita/despesa), valor, data e
contraparte (quem mandou/recebeu). Como e' um documento bem mais simples que cupom,
talvez o Haiku leia tao bem quanto o Sonnet por ~1/3 do preco - e' isso que este
script mede, com dado REAL, antes de decidir mandar comprovante->Haiku no garfo.

100% ISOLADO: nao importa o agente, NAO toca no banco, NAO registra nada, nao mexe
em nenhum arquivo do fluxo. So' chama os dois modelos pra comparar e imprime.

Uso (local ou Render Shell, com ANTHROPIC_API_KEY na env):
    python -m scripts.comparar_comprovante caminho/comprovante.jpg
    python -m scripts.comparar_comprovante c1.jpg c2.pdf c3.png   (varios)
"""
import base64
import json
import mimetypes
...
# usa as MESMAS envs da producao (cai nos padroes se nao setadas)
...
def _bloco_midia(b64, media_type):
...
def extrair(client, modelo_id, b64, media_type):
    resp = client.messages.create(
        model=modelo_id, max_tokens=600,
        messages=[{"role": "user", "content": [
            _bloco_midia(b64, media_type),
...
def comparar(s, h):
    """Confere campo a campo. valor e data sao os CRITICOS (dinheiro)."""
...
def rodar_um(client, caminho):
    ...
    for nome, mid in MODELOS:
        dados, usage = extrair(client, mid, b64, media_type)
...
def main():
    if len(sys.argv) < 2:
        print("uso: python -m scripts.comparar_comprovante <comprovante.jpg|pdf> [outros...]")
        return
    ...
    print("\nLeitura: 'valor' e 'data' sao os CRITICOS (e' dinheiro). Se nao houver")
    print("nenhum DIVERGE-CRITICO num punhado de comprovantes reais, o Haiku le' o")
    print("comprovante tao bem quanto o Sonnet -> o garfo pode mandar comprovante->Haiku")
    print("e cortar ~3x o custo da leitura. DIVERGE-LEVE (so' nome/forma) e' toleravel.")
