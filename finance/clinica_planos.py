...
def salvar(c, conta_id: int, *, plano_id: int | None = None, lead: int | None, evento_id: int | None,
    ...
    d_ass = cas.desconto_procedimento(c, conta_id, lead, fone, paciente)[0]
    # o efetivo inclui o Pix: o assinante com o Pix da clínica também não pede o dono
    teto_ass = 100 * (1 - (1 - d_ass / 100) * (1 - min(pix, cfg["pix_desconto"]) / 100)) if d_ass else 0.0
    teto = max(cfg["teto_desconto"], teto_ass)
