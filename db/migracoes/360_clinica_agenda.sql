...
-- Nada de queixa, diagnóstico ou exame. O `titulo` diz só "Nome · consulta" (nunca
-- o procedimento): é ele que a agenda de sempre, o Cockpit e o .ics mostram.
--
-- NÃO É VISITA. Com prospeccao_id e sem tipo_evento, a linha caberia na definição
-- de visita do app (Cockpit, relatórios, .ics). `finance/visita.sql_e_visita` e o
-- Cockpit passam a excluir linha com `situacao`.
--
...
alter table public.eventos_agenda
  ...
  -- a observação da recepção NUNCA vai em `descricao`: o .ics da agenda de sempre
  -- publica a descrição pro cliente. Aqui ela fica só na tela da clínica.
...
create table if not exists public.clinica_agenda_config (
  ...
  confirmacao_hora smallint not null default 10 check (confirmacao_hora between 7 and 18),
