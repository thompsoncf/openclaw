...
def agendar_visita(pool, conta_id: int, membro_id: int, lead_id: int, *, data: str, hora: str,
    ...
    with pool.connection() as c:
        ...
        lead = c.execute("select coalesce(nullif(contato,''), nullif(empresa,''), 'Cliente'), "
                         "coalesce(whatsapp, telefone, ''), evento_convidados "
                         "from prospeccao where id=%s and conta_id=%s",
                         (lead_id, conta_id)).fetchone()
    quem, numero = (lead[0], lead[1]) if lead else ("Cliente", "")
    # QUANTA GENTE VEM, copiado do lead (13/09/2026). O cliente já disse isso pra
    # quem atendeu — o compromisso da visita é que nascia sem, e a coluna
    # "Convid." do relatório vinha "—" pra visita que o sistema já sabia medir.
    ...
    # cópia; ver a coluna `festa` lá.
    n_conv = lead[2] if lead and len(lead) > 2 else None
