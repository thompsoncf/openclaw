...
from contas.equipe import (caps_do_papel as _caps_do_papel,
                           destino_barrado as _destino_barrado,
                           rotas_do_papel as _rotas_do_papel)
...
async def _gate_permissoes(request: Request, call_next):
    ...
    if papel and papel != "dono":
        ...
        p = request.url.path
        # o destino mora em contas.equipe, junto do CAPS e da whitelist: escrito
        # à mão aqui, um papel novo ganhava rota e continuava caindo no /trocar
        home = _destino_barrado(papel)
...
from web.painel_respostas import router as respostas_router
...
def _iniciar_poller_email() -> None:
    ...
    def _loop():
        ...
        while True:
            ...
            try:
                # Produtos da clínica (migração 386): o lembrete da reposição.
                from finance import clinica_produtos as _cpr
                _pr = _cpr.rodar(pool)
                if _pr["recompra"]:
                    log.info("poller: ciclo #%d — produtos: %d lembrete(s) de reposição", ciclo, _pr["recompra"])
            except Exception as e:  # noqa: BLE001
                log.info("poller: ciclo #%d — produtos falhou: %s: %s", ciclo, type(e).__name__, e)
