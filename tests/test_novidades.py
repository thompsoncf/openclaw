...
def test_o_publico_clinica_e_exatamente_o_nicho_clinica():
    """O aviso da tela Hoje (344) fala de uma tela que só abre pro perfil clínica."""
...
def pool():
    ...
    with p.connection() as c:
        ...
        # 344 amplia pela quinta vez, com `clinica` (a tela Hoje da clínica).
        c.execute((BASE / "344_novidade_hoje_da_clinica.sql").read_text(encoding="utf-8"))
...
# ---------------- 260: a IA marcando visita (14/09/2026)

def _ia_visita(pool) -> dict:
    with pool.connection() as c:
        c.execute((BASE / "260_novidade_ia_marca_visita.sql").read_text(encoding="utf-8"))
        c.commit()
        r = c.execute("""select tipo, publico, pra_quem, resumo, link, corpo, id
