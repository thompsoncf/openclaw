...
# ═════════════════════════════════════════════════════════════════════════════
# CARRINHO / PEDIDO AVULSO (delivery)
# ═════════════════════════════════════════════════════════════════════════════

def _row_pedido(c, pedido_id: int) -> dict | None:
    """Helper: lê uma linha de pedido e devolve dict."""
    r = c.execute(
...
def _recalcular(c, pedido_id: int) -> dict:
    """Recalcula subtotal (soma dos itens), aplica taxa de entrega e grava total."""
    # subtotal = soma dos itens
    r = c.execute(
        "select coalesce(sum(total_centavos),0) from pedido_itens where pedido_id = %s",
        (pedido_id,),
    ).fetchone()
    subtotal = int(r[0] or 0)
    # taxa de entrega do fornecedor (snapshot)
    rf = c.execute(
        """select coalesce(co.taxa_entrega_centavos,0)
           from pedidos p join contas co on co.id = p.fornecedor_id
           where p.id = %s""",
        (pedido_id,),
    ).fetchone()
    ...
    total = subtotal + taxa
    c.execute(
        """update pedidos set subtotal_centavos=%s, taxa_centavos=%s, total_centavos=%s
           where id=%s""",
        (subtotal, taxa, total, pedido_id),
    )
    return {"subtotal_centavos": subtotal, "taxa_centavos": taxa, "total_centavos": total}


def obter_ou_criar_rascunho(pool, cliente_id: int, fornecedor_id: int,
                            canal: str = "web") -> int:
    """Retorna o id do pedido em RASCUNHO (carrinho aberto) do cliente nesse
    fornecedor. Se não existir, cria um. Um carrinho aberto por cliente+fornecedor."""
    with pool.connection() as c:
        r = c.execute(
        ...
        rid = c.execute(
            """insert into pedidos (cliente_id, fornecedor_id, status, canal)
...
def adicionar_item(pool, pedido_id: int, produto_id: int,
    ...
    with pool.connection() as c:
        ...
        # já existe no carrinho? soma
        from decimal import Decimal
        ex = c.execute(
            "select id, quantidade from pedido_itens where pedido_id=%s and produto_id=%s",
        ...
        if ex:
            ...
            c.execute(
                """update pedido_itens set quantidade=%s, preco_unit_centavos=%s,
            ...
            c.execute(
                """insert into pedido_itens
                     (pedido_id, produto_id, nome, unidade, quantidade,
                      preco_unit_centavos, total_centavos)
                   values (%s,%s,%s,%s,%s,%s,%s)""",
                (pedido_id, produto_id, nome, unidade, Decimal(str(quantidade)),
                 preco_unit, total_item),
            )
        totais = _recalcular(c, pedido_id)
        c.commit()
    return {"ok": True, "totais": totais}


def atualizar_quantidade(pool, pedido_id: int, item_id: int,
                         quantidade: float) -> dict:
    """Muda a quantidade de um item. Quantidade 0 ou menos remove."""
    with pool.connection() as c:
        ped = _row_pedido(c, pedido_id)
        if not ped or ped["status"] != "rascunho":
            return {"ok": False, "erro": "pedido nao esta em edicao"}
        it = c.execute(
            "select produto_id, preco_unit_centavos from pedido_itens where id=%s and pedido_id=%s",
            (item_id, pedido_id),
        ).fetchone()
        if not it:
            return {"ok": False, "erro": "item nao encontrado"}
        if float(quantidade) <= 0:
            c.execute("delete from pedido_itens where id=%s", (item_id,))
        else:
            from decimal import Decimal
            preco_unit = int(it[1] or 0)
            total_item = int(round(float(quantidade) * preco_unit))
            c.execute(
                "update pedido_itens set quantidade=%s, total_centavos=%s where id=%s",
...
def remover_item(pool, pedido_id: int, item_id: int) -> dict:
    """Remove um item do carrinho."""
    return atualizar_quantidade(pool, pedido_id, item_id, 0)


def ver_carrinho(pool, pedido_id: int) -> dict | None:
    """Retorna o pedido com seus itens e totais. None se não existe."""
    with pool.connection() as c:
        ped = _row_pedido(c, pedido_id)
        if not ped:
            return None
        itens = c.execute(
            """select id, produto_id, nome, unidade, quantidade,
                      preco_unit_centavos, total_centavos
               from pedido_itens where pedido_id=%s order by id""",
            (pedido_id,),
        ).fetchall()
        ped["itens"] = [
        ...
        # mínimo do fornecedor (pra UI avisar)
        rf = c.execute(
        ...
        ped["pedido_minimo_centavos"] = int(rf[0] or 0) if rf else 0
    return ped


def fechar_pedido(pool, pedido_id: int, endereco_entrega: str | None = None,
                  obs: str | None = None) -> dict:
    """Cliente fecha o carrinho (rascunho -> aguardando). Valida: tem itens E
    atinge o mínimo do fornecedor. Grava endereco/obs. Retorna {ok} ou
    {ok:false, erro, faltam_centavos}."""
    with pool.connection() as c:
        ped = _row_pedido(c, pedido_id)
        if not ped:
            return {"ok": False, "erro": "pedido nao encontrado"}
        if ped["status"] != "rascunho":
            return {"ok": False, "erro": "pedido ja foi enviado"}
        # tem itens?
        n = c.execute(
            "select count(*) from pedido_itens where pedido_id=%s", (pedido_id,)
        ).fetchone()[0]
        if not n:
            return {"ok": False, "erro": "carrinho vazio"}
        totais = _recalcular(c, pedido_id)
        # mínimo do fornecedor (sobre o SUBTOTAL, sem a taxa)
        rf = c.execute(
            """select coalesce(co.pedido_minimo_centavos,0)
               from pedidos p join contas co on co.id=p.fornecedor_id
               where p.id=%s""",
        ...
        # ok: envia
        c.execute(
            """update pedidos set status='aguardando',
                   endereco_entrega=coalesce(%s, endereco_entrega),
                   obs=coalesce(%s, obs)
               where id=%s""",
            (endereco_entrega, obs, pedido_id),
        )
        c.commit()
    return {"ok": True, "totais": totais}


def listar_pedidos_fornecedor(pool, fornecedor_id: int,
                              status: str | None = None) -> list[dict]:
    """Lista os pedidos do fornecedor. Aguardando primeiro (precisa de ação)."""
    with pool.connection() as c:
        q = """select p.id, p.cliente_id, ct.nome, p.status, p.canal,
                      p.subtotal_centavos, p.taxa_centavos, p.total_centavos,
                      p.endereco_entrega, p.obs, p.criado_em
               from pedidos p join contas ct on ct.id = p.cliente_id
               where p.fornecedor_id=%s and p.status != 'rascunho'"""
        args = [fornecedor_id]
        if status:
            q += " and p.status=%s"
            args.append(status)
        q += """ order by case p.status when 'aguardando' then 0
...
def mudar_status_pedido(pool, fornecedor_id: int, pedido_id: int,
                        novo_status: str) -> dict:
    """Fornecedor muda o status (confirmar/em_entrega/entregue/cancelado).
    Marca timestamps."""
    ...
    with pool.connection() as c:
        r = c.execute(
            "select status from pedidos where id=%s and fornecedor_id=%s",
        ...
        args.extend([pedido_id, fornecedor_id])
        c.execute(
            f"update pedidos set {', '.join(sets)} where id=%s and fornecedor_id=%s",
            tuple(args),
        )
        c.commit()
